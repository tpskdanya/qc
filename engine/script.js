document.addEventListener("DOMContentLoaded", () => {
  // Получаем код филиала из URL (например /rnd)
  let branchCode = window.location.pathname.split('/')[1] || 'rnd';
  ...
  // Загрузка конфига филиала
  async function loadBranchConfig() {
    try {
      const response = await fetch(`/cities/${branchCode}/config.json`);
      currentConfig = await response.json();
      
      // Обновляем заголовок
      document.querySelector('.title').innerHTML = `Калькулятор зарплаты <span class="beta">:)</span> — ${currentConfig.branchName}`;
      
      populateCities();
      if (citySel.options.length > 0) populateGrades(citySel.value);
    } catch (e) {
      console.error("Не удалось загрузить config филиала", e);
      alert("Ошибка загрузки данных филиала. Проверьте интернет.");
    }
  }
  ...
  function populateCities() {
    citySel.innerHTML = "";
    Object.keys(currentConfig.cities).forEach(cityName => {
  ...
  // Заполнение грейдов
  function populateGrades(cityName) {
    gradeSel.innerHTML = "";
    const grades = currentConfig.cities[cityName] || {};
  ...
  function calculate() {
    ...
    const baseRate = currentConfig.cities[cityName]?.[grade] || 0;
    const rate = format === "MAU" ? baseRate * currentConfig.coefficients.mau : baseRate;
